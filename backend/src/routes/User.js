    ...
    try {
        if (userConrtoller.correctLoginData(req.body)) {
            ...
            if (foundUser) {
                ...
                const passwordMatch = await userConrtoller.checkPassword(
                    req.body.password,
                    foundUser.password,
                )
                if (passwordMatch) {
                    ...
                    return res
                        .status(400)
                        .send('Bad request: password is incorrect')
                }
            } else {
                return res
                    .status(400)
                    .send('Bad request: email or password is incorrect')
            }
        } else {
            return res
                .status(400)
                .send('Bad request, something went wrong in finding the user')
        }
    } catch (error) {
